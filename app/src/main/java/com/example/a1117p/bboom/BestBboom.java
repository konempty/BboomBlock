...
public class BestBboom extends Fragment {
    ...
    public View onCreateView(@NonNull LayoutInflater inflater, @Nullable ViewGroup container, @Nullable Bundle savedInstanceState) {
        View view = inflater.inflate(R.layout.bestbboom_view, container, false);
        listView = view.findViewById(R.id.best_list);
                    ...
                    new Thread(new Runnable() {
                        ...
                        public void run() {
                            ...
                            if (listViewAdapter.getCount() > 0) {
                                ...
                                last_post = ((Custom_Item) listViewAdapter.getItem(listViewAdapter.getCount() - 1)).getPostno();
                                boolean isSerarched = false;
                                while (count < 15) {
                                    Posts_count += inc_v;
                                    try {
                                        document = Jsoup.connect("http://m.bboom.naver.com/best/moreList.json?likeBandNo=&subCategory=&length=" + String.valueOf(Posts_count)).get();
                                    } catch (IOException e) {
                                        e.printStackTrace();
                                    }
